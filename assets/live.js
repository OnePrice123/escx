...
function toConflict(d, builtAt) {
  ...
  return {
    ...
    /* Имя пары СОБИРАЕТСЯ ИЗ КОДОВ СТОРОН, а не берётся из витрины.
       Пайплайн присылает готовую русскую строку, и раньше она уходила во все
       шестнадцать языков: немец и китаец видели «Россия — Украина» кириллицей
       на английском интерфейсе. Названия стран знает сам браузер — Intl умеет
       их на всех наших языках, и переводить руками сорок стран не нужно.
       Русская строка остаётся запасом, если кода нет в таблице. */
    sideA: d.side_a || null,
    sideB: d.side_b || null,
    ...
    /* ДЕНЬ ДАННЫХ, а не время сборки. Это разные вещи: сборка идёт каждую
       ночь, а числа относятся к последнему дню с полным составом источников,
       и он может быть вчерашним. Показывать время сборки рядом с числами
       значит выдавать вчерашний срез за сегодняшний. */
    ...
    // Начало глубокого ряда: у него собственная точка отсчёта (начало покрытия
    // UCDP), не выводимая из суточного ряда.
    scalesFrom: d.series_all_from || null,
