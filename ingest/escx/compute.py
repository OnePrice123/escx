...
SERIES_DAYS = 1095    # сколько последних дней накала писать в витрину (три года)
#
# Было 90 — ровно под спарклайн. Этого хватало, пока график был один, но
# недельный и месячный масштабы из 90 точек не собрать: месяц даёт три точки.
# Три года при двадцати диадах — это 22 тысячи строк, для SQLite ничто.
#
# ВАЖНО про сравнимость вглубь. Состав блоков по годам разный: до 2026 года
# есть кинетика UCDP и нет медиапотока, в 2026-м наоборот. Поэтому вместе с
# накалом наружу отдаётся покрытие по дням — читатель обязан видеть, что
# именно измерялось, иначе длинный график молча склеит несравнимое.
...
# ОМП, в пайплайне нет и быть не может. Она существует, чтобы у шкалы был
# названный потолок и запас хода: пока «Расширенная война» была последней,
# идущая война упиралась в край, и дальнейшее ухудшение конфликта не отражалось
# в числе вообще. Сто означает не «очень плохо», а ровно одно событие.
#
# Почему ОМП, а не «ядерный удар». Ядерное оружие есть у девяти государств, и в
# нашем реестре ядерная сторона лишь у части пар. Потолок, недостижимый ПО
# ОПРЕДЕЛЕНИЮ для Египта с Эфиопией или Греции с Турцией, ломает сравнимость:
# у одной пары сотня означала бы событие, у другой — ничего. Химическое оружие
# применимо любым государством, и случаи были реальные, а не гипотетические, —
# поэтому категория взята шире, а ядерное остаётся крайним случаем внутри неё.
PHASES = {0: "Нормализация", 1: "Напряжённость", 2: "Кризис",
          3: "Вооружённые инциденты", 4: "Ограниченный конфликт",
          5: "Война", 6: "Расширенная война",
          7: "Применение оружия массового поражения"}
...
def compute(con, *, days: int = SERIES_DAYS, today: date | None = None) -> dict:
    ...
    for r in con.execute("SELECT series_key, as_of, value FROM series "
        ...
        if kind in ("obs", "seen"):
            ...
            #
            # obs — весь трафик зоны, гражданский в том числе; только он на этот
            # вопрос и отвечает. seen — прежний ряд, где то же самое считали по
            # военной ленте и потому не считали вовсе; читается ради уже
            # накопленных снимков, новые пишутся только как obs.
            if r["value"]:
